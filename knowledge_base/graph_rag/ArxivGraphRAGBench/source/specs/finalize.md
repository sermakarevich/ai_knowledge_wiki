...
- Create a successor finalize bead: `fleet bd create "ArxivGraphRAGBench finalize: verify + synthesize" --cwd /Users/sergii/.ai --coder claude --model sonnet -p 1 -t task --body-file /Users/sergii/.ai/knowledge/research/ArxivGraphRAGBench/source/specs/finalize.md --deps "<the still-open bead ids>" --silent`
- Close your own bead: `bd close <own-id> --reason "rearmed as <new-id>: chunks still in flight"`
- Stop.

## Step 2: Verify every wiki page

Expected pages (see `/Users/sergii/.ai/knowledge/research/ArxivGraphRAGBench/source/chunks.json` for the chunk→page mapping):
...
- Attempt count < 3: delete the bad page file, create ONE retry extract bead reusing `source/specs/NN-extract.md` verbatim: `fleet bd create "ArxivGraphRAGBench chunk NN extract retry" --cwd /Users/sergii/.ai --coder opencode --model ollama-rtx/qwen3.8:27b -p 2 -t task --body-file /Users/sergii/.ai/knowledge/research/ArxivGraphRAGBench/source/specs/NN-extract.md --silent`. Record its id.
...
- `connections.md` — links to related entries elsewhere in this KB. Search the KB for other GraphRAG-related papers (e.g. `research/ArxivGraphRAGSurvey`, `research/ArxivLightRAG`, `research/ArxivHippoRAG`, or similar — check `/Users/sergii/.ai/knowledge/research/` for what actually exists) and link them via `[[research/<Folder>/summary|<Title>]]` style links per the KB's cross-folder wikilink convention. If nothing directly related exists yet, say so briefly rather than inventing links.

## Step 5: Report + close

Write a completion report to `/Users/sergii/.ai/knowledge/research/ArxivGraphRAGBench/source/delegation_report.md`: chunks total (4) / passed first try / requeued (how many rounds) / hand-written after exhausting retries.
...
- Touch only files under `/Users/sergii/.ai/knowledge/research/ArxivGraphRAGBench/` and the beads you create/close.
