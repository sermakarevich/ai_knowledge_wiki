...
You are the finalize worker for the `summary/get_local` fleet pipeline (`ai show summary/get_local` and `ai show summary/get` describe the full convention — read both before writing anything, since you must produce the same output shape as the interactive skill). This is the ONLY validation step in the whole pipeline.
...
Folder: `/Users/sergii/.ai/knowledge/research/ArxivGraphReasoningAgentGRA/`
...
Expected pages (from `/Users/sergii/.ai/knowledge/research/ArxivGraphReasoningAgentGRA/source/chunks.json`):
...
Follow `ai show summary/get` conventions exactly. Read the four wiki pages (small, already ~10:1 compressed) — not the raw source — except to spot-check quality against `source/full_text.txt` if something looks off.
...
Produce, per the shared output conventions in `ai show summary/get`:
...
5. **`questions.md`** — 6-8 retrieval-practice questions (this is a short paper, per the size-scaling table) with answers collapsed (e.g. `<details><summary>Answer</summary>...</details>` or a similar collapsed format used elsewhere in this KB — check an existing `research/*/questions.md` file for the house convention if unsure, but do NOT read unrelated full papers, just peek at one `questions.md` file's structure). Ensure every one of the 4 wiki pages has at least one question — do not draw all questions from pages 1-2.
...
7. **`connections.md`** — links to related entries in this KB. Search `/Users/sergii/.ai/knowledge/structured_papers/graph_rag/` (recently created category — several agentic-GraphRAG papers already live there, e.g. `ArxivARESRAGEvaluation`, `ArxivGraphRAGLinkedInCustomerService`, and others from this same batch scan) for related summaries and link them with path-qualified wikilinks per the skill's convention, e.g. `[[structured_papers/graph_rag/SomeOtherPaper/summary|Some Other Paper]]`. Also note the paper's own related-work citations (ReAct, SWE-agent, GraphRAG, Think-on-Graph) as external connections in prose, not as wikilinks (they are not KB entries).

No git commands — `.ai` auto-syncs.

## Step 5 — Report + close

Write a completion report to `/Users/sergii/.ai/knowledge/research/ArxivGraphReasoningAgentGRA/source/delegation_report.md`: chunks total (4) / passed first try / requeued (how many rounds) / hand-written after exhausting retries.

Then: `bd close <own-id> --reason "wiki complete"`.

## Scope

Touch only files under `/Users/sergii/.ai/knowledge/research/ArxivGraphReasoningAgentGRA/` (all subpaths) and, read-only, files under `/Users/sergii/.ai/knowledge/structured_papers/graph_rag/` for connections.md linking. Do not touch other papers' folders. Do not run `fleet serve restart` or `fleet run`.
