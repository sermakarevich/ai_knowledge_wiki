# Source provenance

- **Title:** PersonalAI 2.0: Enhancing knowledge graph traversal/retrieval with planning mechanism for Personalized LLM Agents
- **Authors:** Mikhail Menschikov, Matvey Iskornev, Alexander Kharitonov, Alina Bogdanova, Mikhail Belkin, Ekaterina Lisitsyna, Artyom Sosedka, Victoria Dochkina, Ruslan Kostoev, Ilia Perepechkin, Evgeny Burnaev
- **Venue/Year:** 2026 (arXiv preprint)
- **URL:** https://arxiv.org/abs/2605.13481
- **PDF:** https://arxiv.org/pdf/2605.13481
- **Pages:** 37
- **Retrieved:** 2026-08-21
- **Binary not stored in git:** PDF is 7.3 MB, over the 2 MB size guard in `ai show summary/get` — the original is not copied into this repo. Extracted plain-text chunks used for wiki generation live in `source/chunks/`.
