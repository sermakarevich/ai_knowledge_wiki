...
You are the finalize worker for a KB paper-summarization pipeline (`ai show summary/get_local`). This is the ONLY validation step in the whole run. Read `ai show summary/get` and `ai show summary/get_local` first for full conventions if unfamiliar — but the essential rules are repeated below.
...
No git commands anywhere in this task — `.ai` auto-syncs on its own schedule.
