...
Seven local-model workers have (or will have) each extracted one wiki page for the LLM-wiki folder `/Users/sergii/.ai/knowledge/research/ArxivGraphRAGSurvey/` from "Graph Retrieval-Augmented Generation: A Survey" (Peng et al., 2024, arXiv:2408.08921). This is the ONLY validation step in the whole pipeline -- no per-chunk validation happened. You must verify every page, retry/hand-write any bad ones, then synthesize the remaining artifacts.
...
- Create a successor finalize bead: `fleet bd create "ArxivGraphRAGSurvey finalize: verify + synthesize" --cwd /Users/sergii/.ai --coder claude --model sonnet -p 1 -t task --body-file /Users/sergii/.ai/knowledge/research/ArxivGraphRAGSurvey/source/specs/finalize.md --deps "<the still-open bead ids>" --silent`
- Close your own bead: `bd close <own-id> --reason "rearmed as <new-id>: chunks still in flight"`
- Stop.

### Step 2: Verify every wiki page

The 7 expected wiki pages (see `/Users/sergii/.ai/knowledge/research/ArxivGraphRAGSurvey/source/chunks.json` for the chunk-to-page mapping and source section coverage):
...
- Attempt count < 3: delete the bad wiki page file, create ONE retry extract bead reusing `/Users/sergii/.ai/knowledge/research/ArxivGraphRAGSurvey/source/specs/NN-extract.md` verbatim: `fleet bd create "ArxivGraphRAGSurvey chunk NN extract (retry)" --cwd /Users/sergii/.ai --coder opencode --model ollama-rtx/qwen3.8:27b -p 2 -t task --body-file /Users/sergii/.ai/knowledge/research/ArxivGraphRAGSurvey/source/specs/NN-extract.md --silent`. Record its id.
...
Write a completion report to `/Users/sergii/.ai/knowledge/research/ArxivGraphRAGSurvey/source/delegation_report.md`: chunks total (7) / passed first try / requeued (how many rounds) / hand-written after exhausting retries.

Then `bd close <own-id> --reason "wiki complete"`.

## Tests

- `ls /Users/sergii/.ai/knowledge/research/ArxivGraphRAGSurvey/wiki/*.md` shows 7 files
- `ls /Users/sergii/.ai/knowledge/research/ArxivGraphRAGSurvey/*.md` shows index.md, summary.md, digest.md, explainer.md, questions.md, critical_thinking.md, connections.md
- `cat /Users/sergii/.ai/knowledge/research/ArxivGraphRAGSurvey/source/delegation_report.md` exists and reports counts
...
- Touch ONLY files under `/Users/sergii/.ai/knowledge/research/ArxivGraphRAGSurvey/` and the beads you create/close.
