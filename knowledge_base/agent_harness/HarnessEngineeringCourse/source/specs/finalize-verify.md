...
2. **Verify each page** (`/Users/sergii/.ai/knowledge/papers/HarnessEngineeringCourse/wiki/*.md`): exists, >40 lines, has `**In one sentence:**` + 5+ key bullets, covers the WHOLE chunk (read the TAIL — repetition-loop padding is a known failure), no meta-junk, format contract holds. Build GOOD/BAD lists.
...
4. **Digest skeleton:** `python3 /Users/sergii/.ai/skills/summary/build_digest.py /Users/sergii/.ai/knowledge/papers/HarnessEngineeringCourse` (verbatim copy + FIVE_MOVES markers). Exit 1 → fix named pages, rerun.
5. Append verify section to `/Users/sergii/.ai/knowledge/papers/HarnessEngineeringCourse/source/delegation_report.md`, then `bd close <own-id> --reason "all chunks verified; digest skeleton built"`.

## Tests
- `ls /Users/sergii/.ai/knowledge/papers/HarnessEngineeringCourse/wiki/*.md | wc -l` >= 3; `test -f /Users/sergii/.ai/knowledge/papers/HarnessEngineeringCourse/digest.md`.
